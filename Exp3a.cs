...
//public class Messagebox_Button_MCS
//{
//    [DllImport("User32.dll", SetLastError = true, ThrowOnUnmappableChar = true, CharSet = CharSet.Auto)]
//    public static extern int MessageBox(IntPtr handle, String message, String title, int type);
//}
public class Exp3a : MonoBehaviour
{
    public int repeat = 1;
    ...
    void Resetart()
    {
        ...
        else
        {
            if (current_state == "Experiment")
            {
                ...
                {
                    crowd[random_crowd[i]].SetActive(true);
                    crowd[random_crowd[i]].transform.localPosition = new Vector3(crowd_pos[i][0], crowd[random_crowd[i]].transform.position.y, crowd_pos[i][1]);
                    crowd[random_crowd[i]].transform.rotation = Quaternion.Euler(0, UnityEngine.Random.Range(0, 360), 0);
                }
    ...
    static List<float[]> avaliable_position(int people)
    {
        ...
        for (float i = -7.6f; i <= -0.39f; i = i + 0.4f)
        {
            col.Add(i);
        }
        ...
        for (int m = 0; m < row_L.Count; m++)
        {
            ...
            {
                if (Math.Sqrt(Math.Pow(row_L[m], 2) + Math.Pow(col[n], 2)) > 2.5f)
                {
        ...
        for (int m = 0; m < row_R.Count; m++)
        {
            for (int n = 0; n < col.Count; n++)
            {
                if (Math.Sqrt(Math.Pow(row_R[m], 2) + Math.Pow(col[n], 2)) > 2.5f)
                {
                    pos_R.Add(new float[] { row_R[m], col[n] });
                }
            }
        }
        //foreach (float[] a in pos_L)
        //{
        //    Debug.Log(a[0] + " , " + a[1]);
        //}
